...
c**********************************************************************
c
c          print out normal velocity on slip stream
c
c          DEBUG
c
c           write(*,667)'zsl','dz','dr','vz','vr','vn','vt','slope'
c667        format(8a12)
c
c          END DEBUG
c
	do i=1, np
           ...
           ds = sqrt(dz*dz+dr*dr)
c
c          normalize length to 1
c
           dz = dz/ds
           dr = dr/ds
c
c          project vsl = (vz,vr) on normal = (-dr, dz)
c                                   tang   = ( dz, dr)                  
c
           vn = -(1.+vz(i))*dr + vr(i)*dz
           vt =  (1.+vz(i))*dz+  vr(i)*dr
c
c          rotate by angle of tangent
c
            alfa = atan2(dr,dz)
            vrw = vr(i)
            vzw = 1. + vz(i)
            beta = atan2(vrw,vzw)
            dphi = alfa - beta
c           vn = -(1.+vz(i))*sin(alfa) + vr(i)*cos(alfa)
c
	   write(15,107)zsl(i),vn,dphi
c
c          DEBUG
c
...
c
	...
	real al
c
	character*20 inpstr, s(3) 
        character*20 is(8),slt(0:6)
	character*8 date
	character*10 time
	character*5 zone
	character*89 memstring
c
	memstring ="ps -p $(pgrep -d',' -x VortexCode.exe) -o rss= |
     +  awk '{ i += $1 } END{ print i }'"
c
	write(*,*) memstring
c
...
c
c       read flag if to use contribution from vortec cylinder
c
        read(5,*) inpstr,useVC
        write(*,*)inpstr, useVC
c
...
c
	...
	OPEN(UNIT=15,FORM='FORMATTED',STATUS='UNKNOWN',FILE='sls-vn.DAT')
	OPEN(UNIT=16,FORM='FORMATTED',STATUS='UNKNOWN',FILE='wakei.DAT')
c
c       allocate memory
c
        write(*,*)'***** mem info ***********************************'
        write(*,*)
c
        write(*,*) '***** memory info before allocate in k: *********'
	call system(memstring)
c
        allocate(rsl  (0:np),stat=status)
	allocate(zsl  (0:np),stat=status)
	allocate(ga   (0:np),stat=status)
c
	allocate(rslo (0:np),stat=status)
	allocate(zslo (0:np),stat=status)
	allocate(gao  (0:np),stat=status)
c
...
c
	allocate(vzo  (0:np),stat=status)
	allocate(vro  (0:np),stat=status)
c
	write(*,*)
	write(*,*) '***** memory info after allocate in k: **********'
	call system(memstring)
	write(*,*)
        write(*,*) '***** some derived values ***********************'
        write(*,*)
c
c       ini wake shape
c
...
c	write(*,*)restart
	if(restart)then
	   read(3,*)
	   do l=1,np
	      read(3,*)id,d1,rsl(l),d2,d3,ga(l),(d(ll),ll=1,4)
           end do
...
c
	      case(3)
c
c             accoring vz_ind(r=0) according van Kuik's Eq. (D.9)
c
	      do i=1,np
	         u0    = 1.-0.5*gasivc
	         r0 = i*(rsivc-1.)/np + 1.
	         uwake = 1.-0.5*gasivc*(1.+zsl(i)/
     +          (sqrt(r0*r0+zsl(i)**2)))
	         rsl(i)= sqrt(u0/uwake)
	      end do
c
...
c
	...
	write(*,706)'SL initial  ',nwt,slt(nwt)
	write(*,705)'SL iter mode',modelit,is(modelit)
	write(*,*)
702	format(a20,i6)
703	format(a20,e12.4)
705     format(a20,i10,x,a20) 
706     format(a20,i6,a20)
c
	close(5)
c
...
c
c	write(222,'(7f12.6)')z,zc,r,f1,f2,f3,vzsivcBr
c	vzsivcBr = 0.
c
	return
	end
...
c               
c
c          2026 07 24 FIX (Claude): Vorzeichen. Kontrolle ueber
c          v_r = -(1/r) dPsi/dz mit psiBrCyl ergibt +gasivc*...
c          (Ringformel vrnmBr ist konsistent mit Psi; die alte Version
c          lieferte -v_r, daher '-v_r'-Achse in Fig. 7 des Reports.)
c
...
c
c       2026 07 24 FIX (Claude): Vorzeichen, s. vrsivcBr
...
c
c       2026 07 24 FIX (Claude): Tippfehler 'til' (uninitialisiert)
        f1 = f1/(2.*pi*rsl(n)*sqrt(ztil**2+(1.+rtil)**2))
...
c	if(ie1.ne.0)write(99,*)'error ellk'
c
	return
	end
c--------------------------------------------------------------------------------------
...
c	if(ie1.ne.0.or.ie2.ne.0)write(99,*)'error elle'
c
	return
	end
c
...
c	if(ie1.ne.0.or.ie2.ne.0)write(99,*)'error ellp'
c
	return
	end
c
...
c
  ...
  101 format (a)
c
c      write(77,102) ierr,r1,r2,r3,r4,r5
