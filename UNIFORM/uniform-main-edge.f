C=======================================================================
C  UNIFORM-MAIN-EDGE.F   (edge-refined variant of uniform-main.f)
C
C  Same problem and same iteration as uniform-main.f, with the
C  modifications needed for z1 < 1.d-4 (Table 1 of
C  edge-singularity-proof.tex, rows 1e-5 and 1e-6, 18.09.2026):
C
C  1. u(z) = R(z) - 1 as natural cubic spline in ln z (power-law tail
C     below the first node) instead of R(z) splined in z: the sheet
C     slope tan(theta_s) diverges at the edge and a spline in linear z
C     oscillates there.
C  2. Near-field ring kernels evaluated cancellation-free (RING2):
C     the difference DELTA = R - rho is formed from the u-spline
C     directly, never from the O(1) radii; removes the ~1e-7 noise
C     floor of the standard evaluation (u ~ 1e-5 near the edge).
C  3. Iteration safeguards, all inactive at a converged solution:
C     floor Vzm >= 5e-3, trust-region cap |dgamma| <= 10 percent per
C     iteration, floor R >= 1; optional warm start from a previous
C     run (file start.dat in the format of Gamma.DAT, interpolated
C     in ln z, extrapolated below its range).
C
C  Runs for Table 1: z1=1e-5: NN=106, relax=0.35, relaxg=0.15,
C  start.dat = converged Gamma.DAT of z1=1e-4 (NN=90);
C  z1=1e-6: NN=122, warm from z1=1e-5.  Below z1 ~ 1e-5 the innermost
C  nodes do not converge: theta_s crosses 90 deg near z ~ 1e-6.5 and
C  the single-valued parametrisation r = R(z) ceases to exist (see
C  the note).  Psi_e and cP remain determined to +-1e-4.
C
C  A.P. Schaffarczyk / Claude, HAW Kiel, 18. September 2026
C=======================================================================
C=======================================================================
C  UNIFORM-MAIN.F
C
...
C
C  A.P. Schaffarczyk / Claude, HAW Kiel, 14. September 2026
C=======================================================================
      PROGRAM UNIFORM
      IMPLICIT NONE
      INTEGER NMAX
     ...
     *       R2K(0:NMAX),VZM(NMAX)
      ...
      INTEGER NS,J
      REAL*8 zs(NMAX),rs(NMAX),gs(NMAX),ds,vs,xl0,p,x0
...
C
      ...
      NN     = 40
      z1     = 0.01d0
C  (first node; the global result converges for z1 -> 0, see derivation)
      NITER  = 30
      relax  = 0.7d0
      relaxg = 0.7d0
      NPROF  = 10
...
C
    ...
    1 CONTINUE
C
C  warm start: initial R, gamma from a previous run (file start.dat,
C  format of Gamma.DAT); piecewise linear in ln z, below the range
C  ln(R-1) and gamma extrapolated linearly in ln z
C
      NS = 0
      OPEN(UNIT=17,FILE='start.dat',STATUS='OLD',ERR=5)
      READ(17,*)
    2 CONTINUE
      READ(17,*,END=4) zs(NS+1),rs(NS+1),ds,gs(NS+1),vs
      NS = NS + 1
      IF(NS.LT.NMAX) GO TO 2
    4 CLOSE(17)
      WRITE(6,*)'warm start from start.dat, nodes read: ',NS
      DO 7 I = 1,NN
         x0 = DLOG(ZN(I))
         IF(ZN(I).LE.zs(1)) THEN
            xl0 = DLOG(zs(1))
            p = (DLOG(rs(2)-1.d0)-DLOG(rs(1)-1.d0))
     *          /(DLOG(zs(2))-DLOG(zs(1)))
            RN(I) = 1.d0 + (rs(1)-1.d0)*DEXP(p*(x0-xl0))
            GN(I) = gs(1) + (gs(2)-gs(1))*(x0-xl0)
     *              /(DLOG(zs(2))-DLOG(zs(1)))
         ELSE IF(ZN(I).GE.zs(NS)) THEN
            RN(I) = rs(NS)
            GN(I) = gs(NS)
         ELSE
            DO 8 J = 1,NS-1
               IF(ZN(I).GE.zs(J).AND.ZN(I).LE.zs(J+1)) THEN
                  p = (x0-DLOG(zs(J)))/(DLOG(zs(J+1))-DLOG(zs(J)))
                  RN(I) = rs(J) + p*(rs(J+1)-rs(J))
                  GN(I) = gs(J) + p*(gs(J+1)-gs(J))
               END IF
    8       CONTINUE
         END IF
    7 CONTINUE
...
C
   ...
   10 CONTINUE
      ...
      DO 11 I = 1,NN
         ...
         RN(I) = Rnew(I)
   11 CONTINUE
C  floor: the physical R(z) is >= 1 (wind turbine); inactive at the
C  converged solution
      DO 12 I = 1,NN
         RN(I) = DMAX1(RN(I),1.d0)
   12 CONTINUE
...
C  floor on Vzm: keeps gamma = b/Vzm finite during the iteration
C  (inactive at the converged solution, where Vzm > 0.005 everywhere)
         IF(Vz.LT.5.d-3) Vz = 5.d-3
         VZM(I)  = Vz
         gnew(I) = b/Vz
   20 CONTINUE
      crit2(L) = 0.d0
...
C  trust region: per-iteration change of gamma capped at 10 percent
C  (inactive at convergence; stabilises the innermost nodes)
         ds = gnew(I)-GN(I)
         vs = 0.1d0*DABS(GN(I)) + 1.d-3
         IF(DABS(ds).GT.vs) ds = DSIGN(vs,ds)
         GN(I) = GN(I) + relaxg*ds
   21 CONTINUE
...
C=======================================================================
     ...
     *       R2K(0:NMAX)
      ...
      REAL*8 rhof,zf,CL,CP,zeta,g0,R0,Rz0,res,PT,VZT,VRT,zc,xlnx,uf
      REAL*8 GAMSH,DRADIUS,FUNC
      COMMON/SHEET/X2,GINF,RINF,ZN,RN,GN,ZK,RK,R2K,NN
      COMMON/FUN/pi
      COMMON/FLD/rhof,zf,CL,CP,uf,kind
...
C=======================================================================
      ...
      IMPLICIT NONE
      INTEGER kind
      REAL*8 FUNC,x,rhof,zf,CL,CP,Rx,gx,zeta,P,VZ,VR,f,uf,DELTA
      REAL*8 RADIUS,GAMSH,URAD
      COMMON/FLD/rhof,zf,CL,CP,uf,kind
      EXTERNAL RADIUS,GAMSH,URAD
      Rx   = RADIUS(x)
      gx   = GAMSH(x)
      zeta = zf - x
      IF(zeta.EQ.0.d0) THEN
         ...
         RETURN
      END IF
C  DELTA = R(x) - rho evaluated via u = R - 1 (accurate for
C  |R - rho| << 1: avoids catastrophic cancellation of O(1) numbers)
      DELTA = URAD(x) - uf
      CALL RING2(Rx,rhof,DELTA,zeta,P,VZ,VR)
      IF(kind.EQ.1) THEN
         f = gx*Rx*P
...
C=======================================================================
      ...
      END
C=======================================================================
C  As RING, but with the difference DELTA = R - rho supplied by the
C  caller (accurate near the sheet: d2s and R**2-rho**2 are formed
C  from DELTA, not from the O(1) radii)
C=======================================================================
      SUBROUTINE RING2(R,rho,DELTA,z,P,VZ,VR)
      IMPLICIT NONE
      REAL*8 R,rho,DELTA,z,P,VZ,VR,pi,D2,d2s,kp2,k2,k,Dd,AK,AE,rf,rd
      COMMON/FUN/pi
      IF(rho.LT.1.d-10) THEN
         P  = 0.d0
         VZ = R/(R*R+z*z)**1.5d0
      ...
      AK  = rf(0.d0,kp2,1.d0)
      AE  = AK - k2*rd(0.d0,kp2,1.d0)/3.d0
      P   = ((1.d0+kp2)*AK - 2.d0*AE)/(k*pi*DSQRT(R*rho))
      VZ  = (AK + (DELTA*(R+rho)-z*z)/d2s*AE)/(pi*R*Dd)
      VR  = DABS(z)/(pi*R*rho*Dd)*(-AK + (R*R+rho*rho+z*z)/d2s*AE)
      RETURN
      END
C=======================================================================
...
C=======================================================================
      ...
      REAL*8 ZN(NMAX),RN(NMAX),GN(NMAX),ZK(0:NMAX),RK(0:NMAX),
     *       R2K(0:NMAX)
      REAL*8 X2,GINF,RINF,ptail
      COMMON/SHEET/X2,GINF,RINF,ZN,RN,GN,ZK,RK,R2K,NN
      COMMON/TAIL/ptail
C  u(z) = R(z) - 1 as natural cubic spline in ln z through the nodes
C  (the sheet slope grows like tan(theta_s) -> infinity for z -> 0:
C  a spline in linear z oscillates there; u instead of R keeps full
C  relative accuracy for u << 1); below ZN(1) power-law tail
C  u = (RN(1)-1)*(z/ZN(1))**ptail with ptail from the first two nodes
      DO 1 I = 1,NN
         ZK(I) = DLOG(ZN(I))
         RK(I) = RN(I) - 1.d0
    1 CONTINUE
      CALL spline(ZK(1),RK(1),NN,1.d31,1.d31,R2K(1))
      IF(RN(1).GT.1.d0+1.d-12.AND.RN(2).GT.RN(1)+1.d-14) THEN
         ptail = (DLOG(RN(2)-1.d0)-DLOG(RN(1)-1.d0))
     *           /(DLOG(ZN(2))-DLOG(ZN(1)))
         IF(ptail.LT.0.05d0) ptail = 0.05d0
         IF(ptail.GT.1.5d0)  ptail = 1.5d0
      ELSE
         ptail = 1.d0
      END IF
      RINF = RN(NN)
...
C=======================================================================
      ...
      REAL*8 ZN(NMAX),RN(NMAX),GN(NMAX),ZK(0:NMAX),RK(0:NMAX),
     *       R2K(0:NMAX)
      REAL*8 X2,GINF,RINF,RADIUS,x,y,dy,ptail
      COMMON/SHEET/X2,GINF,RINF,ZN,RN,GN,ZK,RK,R2K,NN
      COMMON/TAIL/ptail
      REAL*8 URAD
      EXTERNAL URAD
      ...
      END
C=======================================================================
C  u(x) = R(x) - 1 from the u-spline in ln z (full relative accuracy)
C=======================================================================
      FUNCTION URAD(x)
      IMPLICIT NONE
      INTEGER NMAX
      ...
      REAL*8 ZN(NMAX),RN(NMAX),GN(NMAX),ZK(0:NMAX),RK(0:NMAX),
     *       R2K(0:NMAX)
      REAL*8 X2,GINF,RINF,URAD,x,y,dy,ptail
      COMMON/SHEET/X2,GINF,RINF,ZN,RN,GN,ZK,RK,R2K,NN
      COMMON/TAIL/ptail
      IF(x.LE.0.d0) THEN
         URAD = 0.d0
      ELSE IF(x.LT.ZN(1)) THEN
         URAD = (RN(1)-1.d0)*(x/ZN(1))**ptail
      ELSE IF(x.GE.X2) THEN
         URAD = RN(NN) - 1.d0
      ELSE
         CALL splint(ZK(1),RK(1),R2K(1),NN,DLOG(x),y,dy)
         URAD = y
      END IF
      RETURN
      END
C=======================================================================
      ...
      REAL*8 ZN(NMAX),RN(NMAX),GN(NMAX),ZK(0:NMAX),RK(0:NMAX),
     *       R2K(0:NMAX)
      REAL*8 X2,GINF,RINF,DRADIUS,x,y,dy,ptail
      COMMON/SHEET/X2,GINF,RINF,ZN,RN,GN,ZK,RK,R2K,NN
      COMMON/TAIL/ptail
      IF(x.LE.0.d0.OR.x.GE.X2) THEN
         DRADIUS = 0.d0
      ELSE IF(x.LT.ZN(1)) THEN
         DRADIUS = (RN(1)-1.d0)*ptail/ZN(1)*(x/ZN(1))**(ptail-1.d0)
      ELSE
         CALL splint(ZK(1),RK(1),R2K(1),NN,DLOG(x),y,dy)
         DRADIUS = dy/x
      END IF
