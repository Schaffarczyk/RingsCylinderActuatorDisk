...
c       Simpson's rule
c
c       Faktor 2: s.  Bontempo Eq (18)
c
	do i  = 2,nd-2,2
	   r  = drd*real(i)
	   f1 = 2.*vzd(r    ,model)
	   f2 = 2.*vzd(r+drd,model)
           a  = a + 2.*r*f1 + 4.*(r+drd)*f2
c	   write(*,*)'r a',r,a
        end do
c   
	f1   = 2.*vzd(1. ,model)
	fdrd = 2.*vzd(drd,model)
	a = (a + f1 + 4.*drd*fdrd)*drd/3.
c        write(*,*)'*** a ', a

	cp = ct*(1. + a)
c
c	write(*,*)'cp ct a',cp, ct, a
c
	return
	end
c
...
c
	if(abs(rs-1.).gt.eps) then
	   f1       = zs/(sqrt(zs**2+(1.+rs)**2))
	   f2       = (rs-1.)/(rs+1.)
	   f2       = ellk(k)-f2*ellp(n,k)
...
c
	use mem
c
	real k,n, theta, delta
c
	dz = z-zsivc
	dr = r-rsivc
c
	if(r.gt.rsivc)delta=0.
	if(r.eq.rsivc)delta=pi
...
c
	f2 = ellk(k)-ellp(n,k)*(r-rsivc)/(r+rsivc)
	f2 = f2*2.*(z-zsivc)/rho2
	theta = f2 + delta 
c
...
c
	f1 = ellk(ks) + ellp(n,ks)*(rsivc-rc)/(rsivc+rc)
...
c
	write(222,'(6f12.6)')z,r,f1,f2,f3,vzsivcBr
c	vzsivcBr = vzsivcBo(z,r)
c
	return
	end
c
...
c
c       Branlard Eq (35.8) - Modified
c
	f1 = 0.5*rn/rp*(ks/(1.-ks))-((2.-ks)/(2.*(1.-ks)))
c	f1 = rn/rp*ks/(1.-ks)-(2.-ks)/(2.*(1.-ks))
c                    ks        ks
...
c
	cosb =1.d0/(dsqrt(1.d0+betam*betam))
	rslm = 0.5d0*(rsl(m)+rsl(m-1))
c
	f1 = -(betamp1-betamm1)/(8.d0*pi)*cosb
	f2 = -ds/(4.d0*pi*rslm)*(-0.25d0+dlog(8.d0*pi*rsl(m)/ds))
c
        vzself = (f1 + f2)*ga(m)
c
	return
...
c
c	 calculates streamfunction at the disk
c
	use mem
c
	real k, ks, f1, f2, rc
...
c
	   ...
	   ds = sqrt(dz*dz+dr*dr)
c
  	   ks = 4.*rc*rr/((rc+rr)**2+zr**2)
	   k  = sqrt(ks)
c
c	   Eq(35.5) branlard: PSI = r * psi ! 
c
	   f1 = ga(n)*ds*sqrt(rc*rr)/(2.*pi)
...
c
        ...
        m0 = 4.*rc*rsivc/ (rc+rsivc)**2
c
	f1 =  zc*sqrt(m)*(1.-m0+m0/m)*ellk(m)
	f2 = -m0/m*elle(m)+(m0-1.)*ellp(m0,m)
c
	write(777,778)'m m0 elle ellk ellp',
     +                m,m0,elle(m),ellk(m),ellp(m0,m)
778	format(a30,5e12.4)
c
c       Psi = r * PSI_theta
c
	f3 = gasivc*sqrt(rsivc*rc)/(2.*pi*m0)
c
	psiBrCyl = f3*(f1+f2)
c
	if(rc.gt.rsivc)then
...
c
	psiBrcyl = psiBrcyl + f4
c
	write(777,777)'zc rc f1 f2 f3 f4 psi',zc,rc,f1,f2,f3,f4,psiBrCyl
777     format(a30,2f8.3,5e12.4)
c
	return
	end
c
...
c
	k = sqrt(k)
c
        elle = rfl(0.,1.-k*k,1.,ie1) - k*k/3.*rdl(0.,1.-k*k ,1.,ie2)	 
